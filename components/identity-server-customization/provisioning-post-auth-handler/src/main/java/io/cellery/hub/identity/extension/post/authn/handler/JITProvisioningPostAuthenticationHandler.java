...
/**
 * This is extension authentication handler responsible for JIT provisioning.
 */
public class JITProvisioningPostAuthenticationHandler extends AbstractPostAuthnHandler {
    ...
    private final static String IDP_UNIQUE_CLAIM_BASE = "http://wso2.org/claims/identity/social/unique/";
    private final static String IDP_COMMON_CLAIM = "http://wso2.org/claims/social/idp-common-attribute";
    ...
    /**
     * To handle the request flow of the extension authentication handler.
     *
     * @param response HttpServlet response.
     * @param context  Authentication context
     * @return Status of this extension authentication handler flow.
        ...
        for (Map.Entry<Integer, StepConfig> entry : sequenceConfig.getStepMap().entrySet()) {
            ...
            if (authenticator instanceof FederatedApplicationAuthenticator) {
                ...
                if (externalIdPConfig != null && externalIdPConfig.isProvisioningEnabled()) {
                    ...
                    username = resolveUsernameUsingCommonAttr(stepConfig, localClaimValues);
    ...
    private String resolveUsernameUsingCommonAttr(StepConfig stepConfig, Map<String, String> localClaimValues)
            throws PostAuthenticationFailedException {

        String username;
        String uniqueIDPClaim = IDP_UNIQUE_CLAIM_BASE + stepConfig
                .getAuthenticatedIdP();

        username = UUID.randomUUID().toString();

        String existingUser = getExistingUser(stepConfig.getAuthenticatedIdP(), localClaimValues,
                uniqueIDPClaim);
        ...
        if (StringUtils.isNotEmpty(existingUser)) {
            ...
            String existingUserWithCommonAttr = getExistingUser(stepConfig.getAuthenticatedIdP(),
                    localClaimValues, IDP_COMMON_CLAIM);
            if (StringUtils.isNotEmpty(existingUserWithCommonAttr)) {
                username = existingUserWithCommonAttr;
            }
        }
        return username;
    }
    ...
    private String getTenantDomainAppendedUserName(String userName, String tenantDomain) {
        ...
        // To handle the scenarios where email comes as username, but EnableEmailUserName is not set true in hub.xml
    ...
    private void callDefaultProvisioningHandler(String username, AuthenticationContext context,
                                                ExternalIdPConfig externalIdPConfig, Map<String, String>
                                                        localClaimValues, StepConfig stepConfig)
            throws PostAuthenticationFailedException {
    ...
    private String getExistingUser(String idpName, Map<String, String> claims, String claimUri) throws
            PostAuthenticationFailedException {
